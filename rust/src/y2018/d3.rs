...
fn apply_claim(
    mut fabric: (Vec<(i32, i32)>, HashMap<i32, bool>),
    claim: &str,
) -> (Vec<(i32, i32)>, HashMap<i32, bool>) {
    ...
    for x in 0..claim.size_x {
        for y in 0..claim.size_y {
            if let Some(square_inch) =
                fabric
                    .0
                    .get_mut(index_2d(x, claim.offset_x, y, claim.offset_y, FABRIC_WIDTH))
            {
                if square_inch.0 >= 1 {
                    fabric.1.insert(square_inch.1, true);
                    fabric.1.insert(claim.id, true);
                }
                if square_inch.0 == 0 && fabric.1.contains_key(&claim.id) == false {
...
fn overlap(claims: &str) -> i32 {
    return claims
        ...
        .fold(
            (
                vec![(0, 0); (FABRIC_WIDTH * FABRIC_WIDTH) as usize],
                HashMap::new(),
            ),
